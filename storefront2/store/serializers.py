...
#    )
   ...
   def claculate_tax(self,product:Product):
...
class CartItemSerializaer(serializers.ModelSerializer):
...
class CartSerializer(serializers.ModelSerializer):
    ...
    items = CartItemSerializaer(many = True,read_only = True)
    total_price = serializers.SerializerMethodField(method_name='total')
...
class AddCartItemSerializer(serializers.ModelSerializer):
    ...
    def validate_product_id(self,value):
       if not Product.objects.filter(pk = value).exists():
           raise serializers.ValidationError('No product with Id this')
       return value
...
class UpdateCarItemSerializer(serializers.ModelSerializer):
   class Meta:
      model = CartItem
      fields = ['quantity']
